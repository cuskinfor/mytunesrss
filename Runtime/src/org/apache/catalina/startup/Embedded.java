...
/**
 * Convenience class to embed a Catalina servlet container environment inside another application.  You must call the methods of this class in the
 * following order to ensure correct operation.
 * <p/>
 * <ul> <li>Instantiate a new instance of this class.</li> <li>Set the relevant properties of this object itself.  In particular, you will want to
 * establish the default Logger to be used, as well as the default Realm if you are using container-managed security.</li> <li>Call
 * <code>createEngine()</code> to create an Engine object, and then call its property setters as desired.</li> <li>Call <code>createHost()</code> to
 * create at least one virtual Host associated with the newly created Engine, and then call its property setters as desired.  After you customize this
 * Host, add it to the corresponding Engine with <code>engine.addChild(host)</code>.</li> <li>Call <code>createContext()</code> to create at least one
 * Context associated with each newly created Host, and then call its property setters as desired.  You <strong>SHOULD</strong> create a Context with
 * a pathname equal to a zero-length string, which will be used to process all requests not mapped to some other Context.  After you customize this
 * Context, add it to the corresponding Host with <code>host.addChild(context)</code>.</li> <li>Call <code>addEngine()</code> to attach this Engine to
 * the set of defined Engines for this object.</li> <li>Call <code>createConnector()</code> to create at least one TCP/IP connector, and then call its
 * property setters as desired.</li> <li>Call <code>addConnector()</code> to attach this Connector to the set of defined Connectors for this object.
 * The added Connector will use the most recently added Engine to process its received requests.</li> <li>Repeat the above series of steps as often as
 * required (although there will typically be only one Engine instance created).</li> <li>Call <code>start()</code> to initiate normal operations of
 * all the attached components.</li> </ul>
 * <p/>
 * After normal operations have begun, you can add and remove Connectors, Engines, Hosts, and Contexts on the fly.  However, once you have removed a
 * particular component, it must be thrown away -- you can create a new one with the same characteristics if you merely want to do a restart.
 * <p/>
 ...
 * @see org.apache.catalina.startup.Catalina#main For a complete example of how Tomcat is set up and launched as an Embedded application.
 */
...
public class Embedded extends StandardService implements Lifecycle {
     ...
     *
     * @param realm Realm implementation to be inherited by all components (unless overridden further down the container hierarchy)
     */
     ...
     *
     * @param address InetAddress to bind to, or <code>null</code> if the connector is supposed to bind to all addresses on this server
    ...
    public Connector createConnector(String address, int port, String protocol) {
        ...
        try {
            ...
            log.error("Couldn't create connector.", e);
        }
    ...
    /**
     * Create, configure, and return a Context that will process all HTTP requests received from one of the associated Connectors, and directed to the
     * specified context path on the virtual host to which this Context is connected.
     * <p/>
     ...
     *
     * @param path    Context path of this application ("" for the default application for this host, must start with a slash otherwise)
    ...
    /**
     * Create, configure, and return an Engine that will process all HTTP requests received from one of the associated Connectors, based on the
    ...
    /**
     * Create, configure, and return a Host that will process all HTTP requests received from one of the associated Connectors, and directed to the
     ...
     * </pre>
     * which will also cause the Host to be started if the Engine has already been started.  If this is the default (or only) Host you will be
     * defining, you may also tell the Engine to pass all requests not assigned to another virtual host to this one:
     * <pre>
    ...
    /**
     * Remove the specified Context from the set of defined Contexts for its associated Host.  If this is the last Context for this Host, the Host
    ...
    /**
     * Remove the specified Engine from the set of defined Engines, along with all of its related Hosts and Contexts.  All associated Connectors are
     * also removed.
     *
    ...
    /**
     * Remove the specified Host, along with all of its related Contexts, from the set of defined Hosts for its associated Engine.  If this is the
     * last Host for this Engine, the Engine will also be removed.
     *
    ...
    /**
     * Gracefully terminate the active use of the public methods of this component.  This method should be the last one called on a given instance of
     * this component.
     *
    ...
    /**
     * Initialize naming - this should only enable java:env and root naming. If tomcat is embeded in an application that already defines those - it
     * shouldn't do it.
     * <p/>
     * XXX The 2 should be separated, you may want to enable java: but not the initial context and the reverse XXX Can we "guess" - i.e. lookup java:
     * and if something is returned assume false ? XXX We have a major problem with the current setting for java: url
     */
