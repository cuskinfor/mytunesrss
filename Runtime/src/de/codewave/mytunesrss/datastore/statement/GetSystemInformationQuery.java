...
public class GetSystemInformationQuery extends DataStoreQuery<SystemInformation> {
    ...
    public SystemInformation execute(Connection connection) {
        try {
            PreparedStatement statement = connection.prepareStatement("SELECT lastupdate AS lastupdate, version AS version, itunes_library_id AS itunes_library_id FROM system_information");
