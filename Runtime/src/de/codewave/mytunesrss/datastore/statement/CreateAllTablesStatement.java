...
public class CreateAllTablesStatement implements DataStoreStatement {
    public void execute(Connection connection) throws SQLException {
        ...
        connection.createStatement().execute(
                "CREATE CACHED TABLE link_track_playlist ( index INTEGER, track_id VARCHAR(2000) NOT NULL, playlist_id VARCHAR(20) NOT NULL, CONSTRAINT fk_linktrackplaylist_trackid FOREIGN KEY (track_id) REFERENCES track (id) ON DELETE CASCADE, CONSTRAINT fk_linktrackplaylist_playlistid FOREIGN KEY (playlist_id) REFERENCES playlist (id) ON DELETE CASCADE )");
        connection.createStatement().execute(
