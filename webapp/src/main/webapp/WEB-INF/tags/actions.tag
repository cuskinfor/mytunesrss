...
<c:if test="${authUser.rss}">
	<a id="fn_rss${index}" class="rss" onclick="self.location.href=$jQ('#fn_rss${index}').attr('href'); return false" href="${permFeedServletUrl}/createRSS/${auth}/<mt:encrypt>${linkFragment}/_cdi=${cwfn:encodeUrl(filename)}.xml</mt:encrypt>" <c:if test="${!config.showRss}">style="display:none"</c:if> title="<fmt:message key="tooltip.rssfeed"/>"><span>RSS</span></a>
    <c:if test="${!config.showRss}"><c:set var="displayMenu" value="true"/></c:if>
</c:if>
<c:if test="${authUser.playlist}">
	<a id="fn_playlist${index}" class="playlist" onclick="self.location.href=$jQ('#fn_playlist${index}').attr('href'); return false" href="${servletUrl}/createPlaylist/${auth}/<mt:encrypt>${linkFragment}/type=${config.playlistType}/_cdi=${cwfn:encodeUrl(filename)}.${config.playlistFileSuffix}</mt:encrypt>" <c:if test="${!config.showPlaylist}">style="display:none"</c:if> title="<fmt:message key="tooltip.playlist"/>"><span>Playlist</span></a>
    <c:if test="${!config.showPlaylist}"><c:set var="displayMenu" value="true"/></c:if>
</c:if>
<c:if test="${globalConfig.flashPlayer && authUser.player}">
	<a id="fn_player${index}" class="flash" <c:if test="${!config.showPlayer}">style="display:none"</c:if> onclick="openPlayer($jQ('#fn_player${index}').attr('href')); return false" href="${servletUrl}/showJukebox/${auth}/<mt:encrypt>playlistParams=<mt:encode64>${linkFragment}</mt:encode64></mt:encrypt>/playerId=" title="<fmt:message key="tooltip.flashplayer"/>"><span>Flash Player</span></a>
...
<c:choose>
    <c:when test="${!empty track && authUser.yahooPlayer && config.yahooMediaPlayer && mtfn:lowerSuffix(config, authUser, track) eq 'mp3'}">
        ...
        <a id="fn_yahoo${index}" class="htrack" href="<c:out value="${mtfn:playbackLink(pageContext, track, null)}"/>" title="<c:out value="${track.name}"/>" style="display:none">
            <img src="${servletUrl}/showImage/${auth}/<mt:encrypt>hash=${track.imageHash}/size=128</mt:encrypt>" style="display:none" alt=""/>
        </a>
    ...
    <c:when test="${empty track && authUser.download && (authUser.maximumZipEntries <= 0 || zipFileCount <= authUser.maximumZipEntries)}">
        <a id="fn_download${index}" class="download" onclick="self.document.location.href=$jQ('#fn_download${index}').attr('href'); return false" href="${servletUrl}/getZipArchive/${auth}/<mt:encrypt>${linkFragment}/_cda=${cwfn:encodeUrl(filename)}.zip</mt:encrypt>" <c:if test="${!config.showDownload}">style="display:none"</c:if> title="<fmt:message key="tooltip.downloadzip"/>"><span>Download</span></a>
    </c:when>
    <c:when test="${empty track && authUser.download && authUser.maximumZipEntries > 0 && zipFileCount > authUser.maximumZipEntries}">
