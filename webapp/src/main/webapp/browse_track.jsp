        ...
        <c:choose>
            <c:when test="${empty sessionScope.playlist}">
                <c:if test="${authUser.rss && config.showRss}">
                    <th class="icon">
                        <a href="${permServletUrl}/createRSS/${auth}/<mt:encrypt key="${encryptionKey}">tracklist=${cwfn:encodeUrl(track.sectionIds)}/lame=${lame}</mt:encrypt>/${mtfn:webSafeFileName(sectionFileName)}.xml">
                ...
                <c:if test="${authUser.playlist && config.showPlaylist}">
                    <th class="icon">
                        <a href="${servletUrl}/createPlaylist/${auth}/<mt:encrypt key="${encryptionKey}">tracklist=${cwfn:encodeUrl(track.sectionIds)}/lame=${lame}</mt:encrypt>/${mtfn:webSafeFileName(sectionFileName)}.${config.playlistFileSuffix}">
                ...
                <c:if test="${authUser.player && config.showPlayer}">
                    <th class="icon">
                        <a href="#" onclick="openPlayer('${appUrl}/flashplayer/xspf_player.swf?autoplay=true&amp;autoload=true&amp;playlist_url=${servletUrl}/createPlaylist/${auth}/<mt:encrypt key="${encryptionKey}">type=Xspf/lame=${lame}/playerRequest=true/tracklist=${cwfn:encodeUrl(track.sectionIds)}</mt:encrypt>/${mtfn:webSafeFileName(sectionFileName)}.xspf')">
...
<tr class="${cwfn:choose(count % 2 == 0, 'even', 'odd')}">
    ...
    <c:choose>
        <c:when test="${empty sessionScope.playlist}">
            <c:if test="${authUser.rss && config.showRss}">
                <td class="icon">
                    <a href="${permServletUrl}/createRSS/${auth}/<mt:encrypt key="${encryptionKey}">track=${track.id}/lame=${lame}</mt:encrypt>/${mtfn:virtualTrackName(track)}.xml">
            ...
            <c:if test="${authUser.playlist && config.showPlaylist}">
                <td class="icon">
                    <a href="${servletUrl}/createPlaylist/${auth}/<mt:encrypt key="${encryptionKey}">track=${track.id}/lame=${lame}</mt:encrypt>/${mtfn:virtualTrackName(track)}.${config.playlistFileSuffix}">
            ...
            <c:if test="${authUser.player && config.showPlayer}">
                <td class="icon">
                    <a href="#" onclick="openPlayer('${appUrl}/flashplayer/xspf_player.swf?autoplay=true&amp;autoload=true&amp;playlist_url=${servletUrl}/createPlaylist/${auth}/<mt:encrypt key="${encryptionKey}">type=Xspf/lame=${lame}/playerRequest=true/track=${track.id}</mt:encrypt>/${mtfn:virtualTrackName(track)}.xspf')">
