        ...
        <c:choose>
            <c:when test="${empty sessionScope.playlist}">
                <c:if test="${authUser.rss && config.showRss}">
                    <th class="icon">
                        <a href="${permServletUrl}/createRSS/${auth}/<mt:encrypt key="${encryptionKey}">tracklist=${cwfn:encodeUrl(track.sectionIds)}/faad2=${faad2}/lame=${lame}</mt:encrypt>/${mtfn:webSafeFileName(sectionFileName)}.xml">
                ...
                <c:if test="${authUser.playlist && config.showPlaylist}">
                    <th class="icon">
                        <a href="${servletUrl}/createPlaylist/${auth}/<mt:encrypt key="${encryptionKey}">tracklist=${cwfn:encodeUrl(track.sectionIds)}/faad2=${faad2}/lame=${lame}</mt:encrypt>/${mtfn:webSafeFileName(sectionFileName)}.${config.playlistFileSuffix}">
                ...
                <c:if test="${authUser.player && config.showPlayer}">
                    <th class="icon">
                        <a href="#" onclick="openPlayer('${appUrl}/flashplayer/xspf_player.swf?autoplay=true&amp;autoload=true&amp;playlist_url=${servletUrl}/createPlaylist/${auth}/<mt:encrypt key="${encryptionKey}">type=Xspf/faad2=${faad2}/lame=${lame}/playerRequest=true/tracklist=${cwfn:encodeUrl(track.sectionIds)}</mt:encrypt>/${mtfn:webSafeFileName(sectionFileName)}.xspf')">
                    ...
                    <th class="icon">
                        <c:choose>
                            <c:when test="${authUser.maximumZipEntries <= 0 || mtfn:sectionTrackCount(track.sectionIds) <= authUser.maximumZipEntries}">
                                ...
                                <img src="${appUrl}/images/download${cwfn:choose(count % 2 == 0, '', '_odd')}.gif" alt="<fmt:message key="tooltip.downloadzip"/>" title="<fmt:message key="tooltip.downloadzip"/>" /></a>
                            </c:when>
                            <c:otherwise>
...
<tr class="${cwfn:choose(count % 2 == 0, 'even', 'odd')}">
    ...
    <c:choose>
        <c:when test="${empty sessionScope.playlist}">
            <c:if test="${authUser.rss && config.showRss}">
                <td class="icon">
                    <a href="${permServletUrl}/createRSS/${auth}/<mt:encrypt key="${encryptionKey}">track=${track.id}/faad2=${faad2}/lame=${lame}</mt:encrypt>/${mtfn:virtualTrackName(track)}.xml">
            ...
            <c:if test="${authUser.playlist && config.showPlaylist}">
                <td class="icon">
                    <a href="${servletUrl}/createPlaylist/${auth}/<mt:encrypt key="${encryptionKey}">track=${track.id}/faad2=${faad2}/lame=${lame}</mt:encrypt>/${mtfn:virtualTrackName(track)}.${config.playlistFileSuffix}">
            ...
            <c:if test="${authUser.player && config.showPlayer}">
                <td class="icon">
                    <a href="#" onclick="openPlayer('${appUrl}/flashplayer/xspf_player.swf?autoplay=true&amp;autoload=true&amp;playlist_url=${servletUrl}/createPlaylist/${auth}/<mt:encrypt key="${encryptionKey}">type=Xspf/faad2=${faad2}/lame=${lame}/playerRequest=true/track=${track.id}</mt:encrypt>/${mtfn:virtualTrackName(track)}.xspf')">
            ...
            <c:if test="${authUser.download && config.showDownload}">
                <td class="icon">
                    <a href="${servletUrl}/downloadTrack/${auth}/<mt:encrypt key="${encryptionKey}">track=${track.id}</mt:encrypt>/${mtfn:virtualTrackName(track)}.${mtfn:suffix(suffixReplacements, track)}">
