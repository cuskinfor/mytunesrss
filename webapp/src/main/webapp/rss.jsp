...
<rss version="2.0"<c:if test="${userAgentPsp}"> xmlns:media="http://search.yahoo.com/mrss/"</c:if>>
    <channel>
        ...
        <image>
            <url>${plainPermServletUrl}/showTrackImage/${auth}/<mt:encrypt key="${encryptionKey}">track=${cwfn:encodeUrl(imageTrackId)}</mt:encrypt></url>
            <title><c:out value="${channel}"/></title>
        ...
        <description><fmt:message key="rssChannelDescription"/></description><c:forEach items="${tracks}" var="track"><c:set var="virtualFileName">${mtfn:virtualTrackName(track)}.${mtfn:suffix(config, authUser, track)}</c:set>
            <item>
                ...
                <enclosure url="${plainPermServletUrl}/playTrack/${auth}/<mt:encrypt key="${encryptionKey}">track=${track.id}/tc=${mtfn:tcParamValue(config, authUser, track)}</mt:encrypt>/${cwfn:encodeUrl(virtualFileName)}"
                           type="${track.contentType}"
                           length="${track.contentLength}"/>
                <c:if test="${userAgentPsp}"><media:thumbnail url="${plainPermServletUrl}/showTrackImage/${auth}/<mt:encrypt key="${encryptionKey}">track=${track.id}</mt:encrypt>" width="160"/></c:if>
            </item></c:forEach>
    </channel>
</rss>
