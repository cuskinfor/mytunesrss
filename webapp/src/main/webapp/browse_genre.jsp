                                    ...
                                    <c:choose>
                                        ...
                                        <c:otherwise>
                                            <c:if test="${authUser.player && config.showPlayer}">
                                                <a id="linkEditPlaylistFlash${loopStatus.index}" class="flash" onclick="openPlayer('${servletUrl}/showJukebox/${auth}/playerId=#ID#/<mt:encrypt key="${encryptionKey}">playlistParams=<mt:encode64>genre=${mtfn:encode64(genre.name)}/filename=${mtfn:virtualGenreName(genre)}.xspf</mt:encode64></mt:encrypt>'); return false;" title="<fmt:message key="tooltip.flashplayer"/>"><span>Flash Player</span></a>
                                            </c:if>
