        ...
        <c:forEach items="${playlists}" var="playlist" varStatus="loopStatus">
            <tr class="${cwfn:choose(loopStatus.index % 2 == 0, 'even', 'odd')}">
                ...
                <c:if test="${authUser.rss && config.showRss}">
                    <td class="icon">
                        <a href="${permServletUrl}/createRSS/${auth}/<mt:encrypt key="${encryptionKey}">playlist=${playlist.id}/faad2=${faad2}/lame=${lame}</mt:encrypt>/${mtfn:webSafeFileName(playlist.name)}.xml">
                ...
                <c:if test="${authUser.playlist && config.showPlaylist}">
                    <td class="icon">
                        <a href="${servletUrl}/createPlaylist/${auth}/<mt:encrypt key="${encryptionKey}">playlist=${playlist.id}/faad2=${faad2}/lame=${lame}</mt:encrypt>/${mtfn:webSafeFileName(playlist.name)}.${config.playlistFileSuffix}">
                ...
                <c:if test="${authUser.player && config.showPlayer}">
                    <td class="icon">
                        <a href="#" onclick="openPlayer('${appUrl}/flashplayer/xspf_player.swf?autoplay=true&amp;autoload=true&amp;playlist_url=${servletUrl}/createPlaylist/${auth}/<mt:encrypt key="${encryptionKey}">playlist=${playlist.id}/playerRequest=true/faad2=${faad2}/lame=${lame}/type=Xspf</mt:encrypt>/${mtfn:webSafeFileName(playlist.name)}.xspf')">
