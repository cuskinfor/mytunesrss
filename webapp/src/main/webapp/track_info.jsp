    ...
    <table cellspacing="0">
        ...
        <c:if test="${authUser.download && config.showDownload}">
            <tr <mt:flipFlop/>>
                ...
                <td>
                    <a href="${fn:replace(servletUrl, 'https', 'http')}/playTrack/${auth}/<mt:encrypt key="${encryptionKey}">track=${track.id}/notranscode=true</mt:encrypt>/${mtfn:virtualTrackName(track)}.${mtfn:suffix(null, null, track)}">
            ...
            <tr>
              <td colspan="2">
                <img alt="${track.name} Album Art"
                  src="${fn:replace(servletUrl, 'https', 'http')}/showTrackImage/${auth}/<mt:encrypt key="${encryptionKey}">track=${track.id}/size=256</mt:encrypt>"
