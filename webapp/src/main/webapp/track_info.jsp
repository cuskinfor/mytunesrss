                        ...
                        <td class="actions">
                            ...
                            <c:if test="${authUser.player && config.showPlayer}">
                                <c:set var="playlistParams">track=${track.id}</c:set>
                                    <a class="flash" style="cursor:pointer" onclick="openPlayer('${servletUrl}/showJukebox/${auth}/<mt:encrypt key="${encryptionKey}">playlistParams=${cwfn:encodeUrl(playlistParams)}</mt:encrypt>/<mt:encrypt key="${encryptionKey}">filename=${mtfn:virtualTrackName(track)}.xspf</mt:encrypt>'); return false;" title="<fmt:message key="tooltip.flashplayer"/>"><span>Flash Player</span></a>
                            </c:if>
