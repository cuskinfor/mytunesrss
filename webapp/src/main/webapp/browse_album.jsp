            ...
            <tr class="${cwfn:choose(loopStatus.index % 2 == 0, 'even', 'odd')}">
                ...
                <td class="albumthumb">
                    <a class="albumthumb" href="#" onclick="return false">
                        <img src="${servletUrl}/showAlbumImage/${auth}/<mt:encrypt key="${encryptionKey}">album=${cwfn:encodeUrl(album.name)}/size=32</mt:encrypt>"/>
                        <span class="albumthumbfull"><img src="${servletUrl}/showAlbumImage/${auth}/<mt:encrypt key="${encryptionKey}">album=${cwfn:encodeUrl(album.name)}/size=128</mt:encrypt>"/></span>
                    </a>
                    <c:choose>
                ...
                <c:choose>
                    <c:when test="${empty sessionScope.playlist}">
                        ...
                        <c:if test="${authUser.player && config.showPlayer}">
                            <td class="icon">
                                <a href="#" onclick="openPlayer('${appUrl}/flashplayer/mediaplayer.swf?file=${servletUrl}/createPlaylist/${auth}/album=${cwfn:encodeUrl(mtfn:encode64(album.name))}/playerRequest=true/tc=${transcodeParam}/type=Xspf/${mtfn:virtualAlbumName(album)}.xspf&displaywidth=256&autostart=true&repeat=list&shuffle=false'); return false">
