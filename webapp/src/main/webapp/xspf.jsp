...
<playlist version="1" xmlns="http://xspf.org/ns/0/">
    ...
    <trackList>
        <c:forEach items="${tracks}" var="item">
            <track>
                <location>${permServletUrl}/playTrack/${auth}/<mt:encrypt key="${encryptionKey}">track=${cwfn:encodeUrl(item.id)}/tc=${param.tc}/playerRequest=${param.playerRequest}</mt:encrypt>/${mtfn:virtualTrackName(item)}.${mtfn:suffix(config, authUser, item)}</location>
