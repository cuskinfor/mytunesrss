...
<c:forEach items="${tracks}" var="item" varStatus="loopStatus">
    {
        ...
        <c:if test="${!empty(item.imageHash)}">"image" : "<mt:escapeJson>${permServletUrl}/showImage/${auth}/<mt:encrypt>hash=${item.imageHash}${imgSizePathParam}</mt:encrypt></mt:escapeJson>",</c:if>
