...
<c:set var="backUrl" scope="request">${servletUrl}/browseArtist/<mt:encrypt>album=${cwfn:encodeUrl(param.album)}/genre=${cwfn:encodeUrl(param.genre)}/page=${param.page}/index=${param.index}</mt:encrypt></c:set>
...
<div class="body">
    ...
    <ul class="links">
        <li>
            <a href="${servletUrl}/browseAlbum/<mt:encrypt>page=${cwfn:choose(empty param.album, param.page, '1')}</mt:encrypt>"><fmt:message key="browseAlbums"/></a>
        </li>
    ...
    <form id="browse" action="" method="post">
        ...
        <table class="select" cellspacing="0">
            ...
            <c:forEach items="${artists}" var="artist" varStatus="loopStatus">
                <tr class="${cwfn:choose(loopStatus.index % 2 == 0, 'even', 'odd')}">
                    ...
                    <td class="album">
                        <a href="${servletUrl}/browseAlbum/<mt:encrypt>artist=${cwfn:encodeUrl(mtfn:encode64(artist.name))}</mt:encrypt>?backUrl=${cwfn:encodeUrl(backUrl)}"> ${artist.albumCount} </a>
                    </td>
                    <td class="tracks">
                        <a href="${servletUrl}/browseTrack/<mt:encrypt>artist=${cwfn:encodeUrl(mtfn:encode64(artist.name))}</mt:encrypt>?backUrl=${cwfn:encodeUrl(backUrl)}"> ${artist.trackCount} </a>
                    </td>
                    <c:choose>
                        <c:when test="${empty sessionScope.playlist}">
                            <c:if test="${authUser.rss && config.showRss}">
                                <td class="icon">
                                    <a href="${servletUrl}/createRSS/<mt:encrypt>auth=${cwfn:encodeUrl(auth)}/artist=${cwfn:encodeUrl(mtfn:encode64(artist.name))}</mt:encrypt>/${mtfn:virtualArtistName(artist)}.xml">
                            ...
                            <c:if test="${authUser.playlist && config.showPlaylist}">
                                <td class="icon">
                                    <a href="${servletUrl}/createPlaylist/<mt:encrypt>auth=${cwfn:encodeUrl(auth)}/artist=${cwfn:encodeUrl(mtfn:encode64(artist.name))}</mt:encrypt>/${mtfn:virtualArtistName(artist)}.${config.playlistFileSuffix}">
                            ...
                            <c:if test="${authUser.player && config.showPlayer}">
                                <td class="icon">
                                    <a href="#" onclick="openPlayer('${appUrl}/flashplayer/xspf_player.swf?autoplay=true&amp;autoload=true&amp;playlist_url=${servletUrl}/createPlaylist/<mt:encrypt>auth=${cwfn:encodeUrl(auth)}/type=Xspf/artist=${cwfn:encodeUrl(mtfn:encode64(artist.name))}</mt:encrypt>/${mtfn:virtualArtistName(artist)}.xspf')">
                                ...
                                <td class="icon">
                                    <c:choose>
                                        <c:when test="${authUser.maximumZipEntries <= 0 || artist.trackCount <= authUser.maximumZipEntries}">
                                            <a href="${servletUrl}/getZipArchive/<mt:encrypt>auth=${cwfn:encodeUrl(auth)}/artist=${cwfn:encodeUrl(mtfn:encode64(artist.name))}</mt:encrypt>/${mtfn:virtualArtistName(artist)}.zip">
                        ...
                        <c:otherwise>
                            <td class="icon">
                                <a href="${servletUrl}/addToPlaylist/<mt:encrypt>artist=${cwfn:encodeUrl(mtfn:encode64(artist.name))}</mt:encrypt>?backUrl=${cwfn:encodeUrl(backUrl)}">
        ...
        <c:if test="${!empty indexPager}">
            ...
            <c:set var="pagerCommand" scope="request">${servletUrl}/browseArtist/<mt:encrypt>page=${param.page}/album=${cwfn:encodeUrl(param.album)}/genre=${cwfn:encodeUrl(param.genre)}</mt:encrypt>?index={index}&amp;backUrl=${cwfn:encodeUrl(param.backUrl)}</c:set>
