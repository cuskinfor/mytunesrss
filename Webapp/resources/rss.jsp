    ...
    <channel>
        ...
        <description><fmt:message key="rssChannelDescription"/></description><c:forEach items="${tracks}" var="track"><c:set var="virtualFileName">${mtfn:virtualTrackName(track)}.${mtfn:suffix(track)}</c:set>
            <item>
                ...
                <c:if test="${userAgentPsp}"><media:thumbnail url="${servletUrl}/showTrackImage/${auth}/<mt:encrypt key="${encryptionKey}">track=${cwfn:encodeUrl(track.id)}</mt:encrypt>" width="160"/></c:if>
            </item></c:forEach>
    </channel>
</rss>
