...
<rss version="2.0"<c:if test="${mediaThumbnails}"> xmlns:media="http://search.yahoo.com/mrss/"</c:if>>
    <channel>
        ...
        <link>${feedUrl}</link>
        <image>
            <url>${servletUrl}/showTrackImage/<mt:encrypt>track=${cwfn:encodeUrl(imageTrackId)}</mt:encrypt></url>
            <title><c:out value="${channel}"/></title>
            <link>${feedUrl}</link>
        </image>
        <description><fmt:message key="rssChannelDescription"/></description><c:forEach items="${tracks}" var="track"><c:set var="virtualFileName">${mtfn:virtualTrackName(track)}.${mtfn:suffix(track)}</c:set>
            <item>
                ...
                <link>${servletUrl}/showTrackInfo/<mt:encrypt>track=${cwfn:encodeUrl(track.id)}/auth=${cwfn:encodeUrl(auth)}</mt:encrypt></link>
                <guid>${servletUrl}/showTrackInfo/<mt:encrypt>track=${cwfn:encodeUrl(track.id)}/auth=${cwfn:encodeUrl(auth)}</mt:encrypt></guid>
                <pubDate>${pubDate}</pubDate>
                <enclosure url="${servletUrl}/playTrack/<mt:encrypt>track=${cwfn:encodeUrl(track.id)}/auth=${cwfn:encodeUrl(auth)}</mt:encrypt>/${cwfn:encodeUrl(virtualFileName)}"
                           type="${track.contentType}"
                           length="${track.contentLength}"/>
                <c:if test="${mediaThumbnails}"><media:thumbnail url="${servletUrl}/showTrackImage/<mt:encrypt>track=${cwfn:encodeUrl(track.id)}</mt:encrypt>" width="160"/></c:if>
            </item></c:forEach>
    </channel>
</rss>
