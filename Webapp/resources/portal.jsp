...
<c:set var="backUrl" scope="request">${servletUrl}/showPortal/<mt:encrypt key="${encryptionKey}">index=${param.index}</mt:encrypt></c:set>
    ...
    <form id="search" action="${servletUrl}/browseTrack" method="post">
        ...
        <table class="portal" cellspacing="0">
            <tr>
                ...
                <td class="links">
                    <a href="${servletUrl}/browseArtist/<mt:encrypt key="${encryptionKey}">page=1</mt:encrypt>" style="background-image:url('${appUrl}/images/library_small.gif');">
        ...
        <c:forEach items="${playlists}" var="playlist" varStatus="loopStatus">
            <tr class="${cwfn:choose(loopStatus.index % 2 == 0, 'even', 'odd')}">
                ...
                <td class="tracks">
                    <c:choose>
                        <c:when test="${playlist.trackCount >= 0}">
                            <a href="${servletUrl}/browseTrack/<mt:encrypt key="${encryptionKey}">playlist=${cwfn:encodeUrl(playlist.id)}</mt:encrypt>/backUrl=${cwfn:encodeUrl(backUrl)}"> ${playlist.trackCount} </a>
                        </c:when>
                        <c:otherwise>
                            &nbsp;
                        </c:otherwise>
                ...
                <c:if test="${authUser.rss && config.showRss}">
                    <td class="icon">
                        <a href="${servletUrl}/createRSS/<mt:encrypt key="${encryptionKey}">auth=${cwfn:encodeUrl(auth)}/playlist=${cwfn:encodeUrl(playlist.id)}</mt:encrypt>/${mtfn:webSafeFileName(playlist.name)}.xml">
                ...
                <c:if test="${authUser.playlist && config.showPlaylist}">
                    <td class="icon">
                        <a href="${servletUrl}/createPlaylist/<mt:encrypt key="${encryptionKey}">auth=${cwfn:encodeUrl(auth)}/playlist=${cwfn:encodeUrl(playlist.id)}</mt:encrypt>/${mtfn:webSafeFileName(playlist.name)}.${config.playlistFileSuffix}">
                            <img src="${appUrl}/images/playlist${cwfn:choose(loopStatus.index % 2 == 0, '', '_odd')}.gif" alt="playlist" /> </a>
                    </td>
                </c:if>
                <c:if test="${authUser.player && config.showPlayer}">
                    <td class="icon">
                        <a href="#" onclick="openPlayer('${appUrl}/flashplayer/xspf_player.swf?autoplay=true&amp;autoload=true&amp;playlist_url=${servletUrl}/createPlaylist/<mt:encrypt key="${encryptionKey}">auth=${cwfn:encodeUrl(auth)}/playlist=${cwfn:encodeUrl(playlist.id)}/type=Xspf</mt:encrypt>/${mtfn:webSafeFileName(playlist.name)}.xspf')">
                    ...
                    <td class="icon">
                        <c:choose>
                            <c:when test="${authUser.maximumZipEntries <= 0 || playlist.trackCount <= authUser.maximumZipEntries}">
                                <a href="${servletUrl}/getZipArchive/<mt:encrypt key="${encryptionKey}">auth=${cwfn:encodeUrl(auth)}/playlist=${cwfn:encodeUrl(playlist.id)}</mt:encrypt>/${mtfn:webSafeFileName(playlist.name)}.zip">
