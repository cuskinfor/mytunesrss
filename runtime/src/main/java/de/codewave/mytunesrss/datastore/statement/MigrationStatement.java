        ...
        if (databaseVersion.compareTo(new Version(MyTunesRss.VERSION)) < 0) {
            ...
            if (databaseVersion.compareTo(new Version("3.0")) < 0) {
                ...
                try {
                    ...
                    // migration for 4.1.0-EAP-5
                    if (databaseVersion.compareTo(new Version("4.1.0-EAP-5")) < 0) {
                        LOG.info("Migrating database to 4.1.0-EAP-5.");
                        MyTunesRssUtils.createStatement(connection, "migrate_4.1.0_eap_5_part_1").execute();
                        MyTunesRssUtils.createStatement(connection, "migrate_4.1.0_eap_5_part_2").execute();
                        MyTunesRssUtils.createStatement(connection, "migrate_4.1.0_eap_5_part_3").execute();
                        databaseVersion = new Version("4.1.0-EAP-5");
                        new UpdateDatabaseVersionStatement(databaseVersion.toString()).execute(connection);
                    }
                    ...
                    // migration for 4.9
                    if (databaseVersion.compareTo(new Version("4.9")) < 0) {
                        LOG.info("Migrating database to 4.9.");
                        MyTunesRssUtils.createStatement(connection, "migrate_4.9").execute();
                        databaseVersion = new Version("4.9");
                        new UpdateDatabaseVersionStatement(databaseVersion.toString()).execute(connection);
                    }
                    // migration for 4.9.4
                    if (databaseVersion.compareTo(new Version("4.9.4")) < 0) {
                        LOG.info("Migrating database to 4.9.4.");
                        MyTunesRssUtils.createStatement(connection, "migrate_4.9.4").execute();
                    ...
                    // migration for 5.0.0
                    if (databaseVersion.compareTo(new Version("5.0.0")) < 0) {
                        LOG.info("Migrating database to 5.0.0.");
                        MyTunesRssUtils.createStatement(connection, "migrate_5.0.0_part_1").execute();
                    ...
                    // migration for 6.1
                    if (databaseVersion.compareTo(new Version("6.1")) < 0) {
                        LOG.info("Migrating database to 6.1.");
                        MyTunesRssUtils.createStatement(connection, "migrate_6.1_part_1").execute();
